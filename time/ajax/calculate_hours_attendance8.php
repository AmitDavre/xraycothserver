	...
	foreach($dates as $d){
		...
		if($res = $dbc->query($sql)){
			while($row = $res->fetch_assoc()){
				...
				$data[$row['id']]['leave_days'] = $row['leave_days'];
				$data[$row['id']]['leave_day'] = $row['leave_day'];
				$data[$row['id']]['leave_paid'] = $row['leave_paid'];
			}
		}
		...
		foreach($data as $k=>$v){
			...
			if($calculate){
				...
				if($v['leave_days'] > 0 && $v['leave_days'] < 0.5){
					$ld = unserialize($v['leave_day']);
					if($normal_hrs+$ld[0] >= $plan){
						$late = 0;
						$early = 0;
						$paid_late = 0;
						$unpaid_late = 0;
