<?
	...
	$data = array();
	$sql = "SELECT COUNT(id) as days, emp_id, ".$lang."_name, SUM(deci_ot1) as ot1, SUM(deci_ot15) as ot15, SUM(deci_ot2) as ot2, SUM(deci_ot3) as ot3, SUM(deci_late) as late, SUM(deci_early) as early, plan, SUM(comp1) as comp1, SUM(comp2) as comp2, SUM(comp3) as comp3, SUM(comp4) as comp4, SUM(comp5) as comp5 FROM ".$cid."_attendance WHERE month = '".$cur_month."' AND status = 1 GROUP BY emp_id";
	if($res = $dbc->query($sql)){
		while($row = $res->fetch_assoc()){
			$step = getEmployeeStep($row['emp_id']);
			//var_dump($step);
			$data[$row['emp_id']]['emp_id'] = $row['emp_id'];
			$data[$row['emp_id']]['emp_name'] = $row[$lang.'_name'];
			$data[$row['emp_id']]['paid'] = 30;
			$data[$row['emp_id']]['ot1'] = $row['ot1'];
			$data[$row['emp_id']]['ot15'] = $row['ot15'];
			$data[$row['emp_id']]['ot2'] = $row['ot2']; 
			$data[$row['emp_id']]['ot3'] = $row['ot3'];
			$data[$row['emp_id']]['late_early'] = $row['late'] + $row['early'];
...
<style>
	...
	xtable.dataTable input:hover {
		background: #ffc !important;
	}
</style>
	
  <h2>
  	<i class="fa fa-table"></i>&nbsp;&nbsp;<?=$lng['Monthly attendance']?>&nbsp;&nbsp;<i class="fa fa-arrow-circle-right"></i>&nbsp;&nbsp;<?=$lng['From']?> <?=date('d-m-Y', strtotime($time_periods['start']))?> <?=$lng['Until']?> <?=date('d-m-Y', strtotime($time_periods['end']))?>
		...
		<div class="tab-content" style="height:calc(100% - 40px); position:relative">
			...
			<div class="tab-pane active" id="tab_attendance" style="height:100%; overflow-y:auto">
				...
				<div style="position:absolute; top:-38px; right:0">
					<button <? if(!$approved){echo 'disabled';}?>  id="sendToPayroll" type="submit" class="btn btn-primary btn-fr">
						<i class="fa fa-paper-plane"></i>&nbsp; <?=$lng['Send to Payroll']?>
					</button>
					<button <? if($approved){echo 'disabled';}?> type="button" id="approveBut" class="btn btn-primary btn-fr approve"><i class='fa fa-thumbs-up'></i>&nbsp; <?=$lng['Approve']?> </button>
					<!--<button id="updateBtn" <? if(!$approved){echo 'disabled';}?>  type="submit" class="btn btn-primary btn-fr">
				...
				<table id="datatable" class="dataTable hoverable selectable nowrap compact" style="clear:both">
					<thead>
						<tr>
							...
							<th class="tac" colspan="1"><? //=$lng['Days']?></th>
							<th class="tac" colspan="4"><?=$lng['Overtime']?> (<?=$lng['Hrs']?>)</th>
							<th class="tac" colspan="2">Leave unPaid<? //=$lng['Absence']?> (<?=$lng['Hrs']?>)</th>
							<th class="tac" colspan="2">Leave Paid<? //=$lng['Absence']?> (<?=$lng['Hrs']?>)</th>
							<th class="tac" colspan="<?=count($compensations)?>">Compensations<? //=$lng['Variable allowances']?></th>
						...
						<tr>
							...
							<th class="tac" style="min-width:30px"><?=$lng['Days']?></th> 
							
							<th class="tac" style="min-width:30px"><?=$lng['OT 1']?></th>
							<th class="tac" style="min-width:30px"><?=$lng['OT 1.5']?></th>
							<th class="tac" style="min-width:30px"><?=$lng['OT 2']?></th>
							<th class="tac" style="min-width:30px"><?=$lng['OT 3']?></th>
							
							<th class="tac" style="min-width:60px"><?=$lng['Absence']?></th>
							<th class="tac" style="min-width:60px"><?=$lng['Late Early']?></th>
							<th class="tac" style="min-width:60px">Personal<? //=$lng['Leave WOP']?></th>
							<th class="tac" style="min-width:60px">Public<? //=$lng['Leave WP']?></th>
					...
					<? if($data){ foreach($data as $key=>$val){ ?>
						<tr>
							<td><?=$val['emp_id']?></td>
							<td><?=$val['emp_name']?></td>
							<td class="tac"><?=$val['days']?></td>
							<td class="tar nopad"><input class="sel float32" type="text" name="<?=$val['emp_id']?>[ot1h]" value="<?=$val['ot1']?>"></td>
							<td class="tar nopad"><input class="sel float32" type="text" name="<?=$val['emp_id']?>[ot15h]" value="<?=$val['ot15']?>"></td>
							<td class="tar nopad"><input class="sel float32" type="text" name="<?=$val['emp_id']?>[ot2h]" value="<?=$val['ot2']?>"></td>
							<td class="tar nopad"><input class="sel float32" type="text" name="<?=$val['emp_id']?>[ot3h]" value="<?=$val['ot3']?>"></td>
							<td class="tar nopad"><input class="sel float32" style="width:80px" type="text" xname="<?=$val['emp_id']?>[absence]" value="-"></td>
							<td class="tar nopad"><input class="sel float32" style="width:80px" type="text" name="<?=$val['emp_id']?>[late_early]" value="<?=round($val['late_early'],2)?>"></td>
							<td class="tar nopad"><input class="sel float32" style="width:80px" type="text" xname="<?=$val['emp_id']?>[xxx]" value="-"></td>
							<td class="tar nopad"><input class="sel float32" style="width:80px" type="text" xname="<?=$val['emp_id']?>[xxx]" value="-"></td>
							<? foreach($compensations as $k=>$v){
									echo '<td class="tac">'.$val['comp'.$k].'</td>';
								} ?>
							<? foreach($var_allow as $k=>$v){ if($allow_cols[$k] == 1){
									echo '<td class="tar nopad"><input style="width:80px" class="sel float52" type="text" name="'.$val['emp_id'].'[var_allow_'.$k.']" value="'.round($val['var_allow_'.$k],2).'"></td>';
								}} ?>
						</tr>
					<? }}else{ ?>
						<tr>
							<td colspan="30" class="dataTables_empty">No data available</td>
						</tr>
					 <? } ?>
					</tbody>
				</table>
	...
	$(document).ready(function() {
		...
		//var acc = <? //=json_encode($_SESSION['rego']['emp_group'])?>;
		...
		var popOverSettings = {
			...
			$(document).on("submit", "#popForm", function(e){
				...
				$.ajax({
					...
					success: function(result){
						//$("#dump").html(result);
						$("#message").html('<div class="msg_alert nomargin">Monthly attendance rejected</div>').hide().fadeIn(300);
			...
			$(document).on('click','#reviewBut', function(e) {
				...
				$.ajax({
					...
					success: function(result){
						//$("#dump").html(result);
						$("#message").html('<div class="msg_success nomargin">Monthly attendance reviewed for payroll</div>').hide().fadeIn(300);
				...
				$.ajax({
					...
					success: function(responce){
						...
						if(responce == 'success'){
							$("body").overhang({
								type: "success",
								message: '<i class="fa fa-check"></i>&nbsp;&nbsp;Monthly attendance approved for payroll',
								duration: 3,
							})
						}else{
							$("body").overhang({
								type: "error",
								message: '<i class="fa fa-exclamation-triangle"></i>&nbsp;&nbsp;Error : ' + responce,
								duration: 4,
							})
						}
			...
			$.ajax({
				...
				success: function(data){
					...
					if(data == 'success'){
						$("body").overhang({
							type: "success",
							message: '<i class="fa fa-check"></i>&nbsp;&nbsp;Data send to payroll successfuly',
							duration: 2,
						})
					}else if(data == 'empty'){
						$("body").overhang({
							type: "error",
							message: '<i class="fa fa-exclamation-triangle"></i>&nbsp;&nbsp;Payroll for this month has not been created yet',
							duration: 4,
						})
					}else if(data == 'locked'){
						$("body").overhang({
							type: "error",
							message: '<i class="fa fa-exclamation-triangle"></i>&nbsp;&nbsp;Payroll for this month is locked already',
							duration: 4,
						})
					}else{
						$("body").overhang({
