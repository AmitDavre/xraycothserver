...
foreach ($emp_en_name as $key => $value) {
	...
	// get default logo
	$sql32444 = "SELECT * FROM " . $cid . "_entities_data where ref = '".$value['entity']."'";
	...
	$emp_first_name2 = $value['firstname'];
	$emp_last_name2 = $value['lastname'];
	$emp_full_name2 = $emp_first_name2 . ' ' . $emp_last_name2;



	if ($langvalue == 'th') {
		$text1 = str_replace('<b>{EMPLOYEE_NAME}</b>', '<b>' . $emp_full_name2 . '</b>', $block1);
		$text1 = str_replace('<b>{COMPANY_NAME}</b>', '<b>' . $en_compname . '</b>', $text1);
		$text1 = str_replace('<b>{GENDER_VALUE}</b>', '<b>' . $title[$value['title']] . '</b>', $text1);
