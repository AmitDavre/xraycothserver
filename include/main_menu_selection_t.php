		...
		<div class="btn-group" style="opacity:0">	
		    <select multiple="multiple" id="selBox-entities">
					...
					<option <? if(in_array($k, explode(',', $_SESSION['rego']['selt_entities']))){echo 'selected';} ?> value="<?=$k?>"><?=$v[$lang]?></option>
					<? }} ?>
		    </select>
		</div>	
		<? } ?>
    ...
    <select multiple="multiple" id="selBox-branches">
			...
			<option <? if(in_array($k, explode(',', $_SESSION['rego']['selt_branches']))){echo 'selected';} ?> value="<?=$k?>"><?=$v[$lang]?></option>
			<? }} ?>
    </select>
    </div>
		<? } ?>
		...
		<? if(count($divisions) > 1){
			...
			<select multiple="multiple" id="selBox-divisions">
				...
				<option <? if(in_array($k, explode(',', $_SESSION['rego']['selt_divisions']))){echo 'selected';} ?> value="<?=$k?>"><?=$v[$lang]?></option>
				<? }} ?>
			</select>
			</div>
		<? }} ?>
		...
		<? if(count($departments) > 1){
			...
			<select multiple="multiple" id="selBox-departments">
				...
				<option <? if(in_array($k, explode(',',$_SESSION['rego']['selt_departments']))){echo 'selected';} ?> value="<?=$k?>"><?=$v[$lang]?></option>
				<? }} ?>
			</select>
			</div>
		<? }} ?>
		...
		<select multiple="multiple" id="selBox-teams">
			...
			<option <? if(in_array($k, explode(',',$_SESSION['rego']['selt_teams']))){echo 'selected';} ?> value="<?=$k?>"><?=$v[$lang]?></option>
			<? }} ?>
    </select>
		</div>
		<? } ?>
